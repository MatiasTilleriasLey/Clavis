...
**De audio a partitura editable, en tu propia red.**

Clavis convierte cualquier audio —un archivo o un link de YouTube / Instagram / TikTok— en una
**partitura editable** (renderizada en el navegador y descargable en PDF, MusicXML y MIDI), con
**separación de instrumentos** para obtener una partitura por instrumento. Multiusuario, open
source y pensado para correr en **red local / VPN privada**.
...
- **Separación de instrumentos** opcional: voz, batería, bajo, guitarra, piano y otros. El
  usuario elige cuáles transcribir (o "Extraer todos"); se genera una partitura por instrumento.
- **Transcripción a partitura**: detecta las notas, la **tonalidad** (y aplica la armadura
  correspondiente) y el **tempo** (♩ = BPM). El piano se escribe en **gran pentagrama** (clave de
  sol + clave de fa).
- **Salidas**: render interactivo en el navegador (OpenSheetMusicDisplay) y descargas en **PDF**,
  **MusicXML** y **MIDI**.
...
Dashboard  ──► subir archivo  ó  pegar link (YouTube/IG/TikTok)
        ...
        │            │
        │            ├─ [si se eligieron instrumentos] Demucs separa en stems
        │            ▼
        │      basic-pitch (audio → MIDI)  →  music21 (tonalidad, tempo, gran pentagrama,
        │                                     metadata → MusicXML)  →  MuseScore (PDF)
        ▼
Partitura guardada por usuario  ──►  ver (OSMD) · descargar PDF/MusicXML/MIDI · editar datos
        (el audio original se borra siempre)
```
...
| Separación | **Demucs** (`htdemucs_6s`, PyTorch, CPU) | Separa la mezcla en 6 stems |
| Audio → MIDI | **basic-pitch** (Spotify, backend ONNX) | Detección de notas |
| MIDI → partitura | **music21** | Tonalidad, tempo, gran pentagrama, metadata → MusicXML |
| Partitura → PDF | **MuseScore 4 CLI** (headless) | Export a PDF |
| Render en navegador | **OpenSheetMusicDisplay** (self-hosteado) | Partitura interactiva |
...
> Demucs (MIT), basic-pitch (Apache-2.0), music21 (BSD/LGPL), Flask y ecosistema (BSD),
> OpenSheetMusicDisplay (BSD-3), PyTorch (BSD), MuseScore (GPLv3).
...
3. En el **dashboard**, subí un archivo o pegá un link. Opcionalmente marcá qué instrumentos
   separar (o "Extraer todos").
